...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        <h1 className="text-3xl md:text-4xl text-text-primary mb-2">🌾 {t('welcome')}</h1>
        <p className="text-text-secondary text-lg">{t('tagline')}</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={<Activity />} label={t('cropsAnalyzed')} value="12,450" trend="+14% this month" />
        <StatCard icon={<Bug />} label={t('diseasesDetected')} value="3,892" trend="+5% this week" />
        <StatCard icon={<TrendingUp />} label={t('predictionsMade')} value="8,104" trend="+22% this month" />
        <StatCard icon={<MessageSquare />} label={t('chatsToday')} value="456" trend="Active now" />
      </div>
      ...
      <div className="bg-green-100 rounded-lg p-5 flex items-start gap-4 border border-green-200">
        ...
        <div>
          <h4 className="font-semibold text-green-900 mb-1">{t('tipOfDay')}</h4>
